...
interface ISnackbarProviderState {
    ...
    theme: Partial<Theme> | ((outerTheme: Theme) => Theme) | undefined;
}

//  Snackbar Provider 用于管理Snackbar
class SnackbarProvider extends Component<
    ISnackbarProviderProps,
    ISnackbarProviderState
> {
    static newInstance: (
        container: any,
    ...
    constructor(props: ISnackbarProviderProps) {
        ...
        this.state = {
            ...
            theme: props.theme || this.defaultTheme,
        };
    ...
    set defaultTheme(newTheme: Theme) {
        this._defaultTheme = newTheme;
    }
    /**
    ...
    show(
        ...
        const { theme, ...opt } = option;
        if (theme && theme!== this.state.theme) {
        ...
        return this.enqueueSnackbar(message, opt);
    }
    ...
    render(): JSX.Element {
        ...
        // 从 state解构出theme
        const { contextValue, theme } = this.state;

        const categ = this.state.snacks.reduce<SnacksByPosition>(
            (acc, current) => {
                    ...
                    {snacks.map((snack) => (
                        <SnackbarItem
                            ...
                            onExited={createChainedFunction([
                                this.handleExitedSnack,
                                this.props.onExited,
                            ])}
                            onEntered={createChainedFunction([
                                this.handleEnteredSnack,
        ...
        return (
            <ThemeProvider theme={theme || themeProp || this.defaultTheme}>
                <SnackbarContext.Provider value={contextValue}>
                    {children}
                    {domRoot
                        ? ReactDOM.createPortal(snackbars, domRoot)
                        : snackbars}
...
SnackbarProvider.newInstance = (
    ...
    function ref(messageInstance: SnackbarProvider) {
        if (!messageInstance) {
            return;
        }
        callback(messageInstance);
    }
    ReactDOM.render(<SnackbarProvider ref={ref} />, domRoot);
};
